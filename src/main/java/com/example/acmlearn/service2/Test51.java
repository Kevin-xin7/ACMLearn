...
public class Test51 {
    ...
    private void dfs(List<Integer> list, int n, int y, HashMap<Integer, List<Integer>> map) {
        if (list.size() == n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!check(i, y, n, map)) continue;
            list.add(i);
            y++;
            dfs(list, n, y, new HashMap<>(map));
            y--;
            map = revert(i, y, map);
    ...
    private boolean check(int x, int y, int n, HashMap<Integer, List<Integer>> map) {
